...
def index():
    ...
    html=f'<!DOCTYPE html><html><head><link rel="stylesheet" type="text/css" href="static/test.css"><title>Music Recommendation System        </title></head>'
    html+=f'<body><br/><h1 class="center">Music Recommendation</h1><br/><p><div style="width: 95%; overflow: hidden; margin:auto;">'
    html+=f'<div class="container" style="width: 550px; height: 350px; float: left; margin:auto; margin-top:18px;"><form action="/hello" method="post" class="center" enctype="multipart/form-data"><br/><br/><p class="center">Upload a Song: <br><br><br><input class="center" type="file" name="file"> <br/><br/><br/><p class="center"><input type="submit" name="form" value="Submit"/></p></form></div></p>'
    html+=f'<div class="container" style="width:550px; height: 350px; float: right; margin:auto"><form action="/songselect" method="post" class="center" enctype="multipart/form-data"><p class="center">Or Select a song from the dataset: <p><select class="center" name="songselection" size="8"></p>'
    for s in names:
        html+=f'<option>{s}</option>'
    html+=f'</select><p class="center"><input type="submit" name="form" value="Submit"/></p></form></div></div></p></body></html>'
    return html
...
def hello():
    ...
    tempflag=0
    for file_name in file_data:
        for songs in data.song_name:
            if songs==file_name:  #song exists in dataset, so no need for extraction
                listtt=rec(file_name)
                tempflag=1
                songname=path+file_name
                if os.path.isfile(songname):
                    os.remove(songname)
        if tempflag==0: #song does not exists in dataset, so extraction needed
            file_name2=ext(file_data)
            listtt=rec(file_name2)
    ...
    char6="'ve "
    char7="'re "
    nfn=nfn.replace('_t_',char1)
    nfn=nfn.replace('_t.',char2)
    nfn=nfn.replace('_s_',char3)
    nfn=nfn.replace('_s.',char4)
    nfn=nfn.replace('_ll',char5)
    nfn=nfn.replace('_ve_',char6)
    nfn=nfn.replace('_re_',char7)
    nfn=nfn.replace('__',' ')
    nfn=nfn.replace('_',' ')
    nfn=nfn[:-4]
    table+=f'<h2>Song Recommendations for "{nfn}" <br><audio controls="controls" src="{url_for("static", filename=file_name)}" type="file/mpeg" /><br></h2><table class="center"><thead><tr><th>Song</th><th>Play Song</th></tr></thead><tbody>'
    for i in range (0,9):
        abc=listtt[i][0]
        audiopath=abc
        abc=abc.replace('___',' & ')
        ...
        char6="'ve "
        char7="'re "
        abc=abc.replace('_t_',char1)
        abc=abc.replace('_t.',char2)
        abc=abc.replace('_s_',char3)
        abc=abc.replace('_s.',char4)
        abc=abc.replace('_ll',char5)
        abc=abc.replace('_ve_',char6)
        abc=abc.replace('_re_',char7)
        abc=abc.replace('__',' ')
        abc=abc.replace('_',' ')
        abc=abc[:-4]
        table+=f'<td>{abc}</td><td><audio controls="controls" src="{url_for("static", filename=audiopath)}" type="file/mpeg" /></td></tr>'
    table+=f'</tbody></table><br/><a href="/">Back Home</a>'
    return table 

@app.route('/songselect', methods=['GET','POST'])
def hi():
    song_query = request.form['songselection']
    import re
    import traceback
    import librosa
    import sklearn
    ...
    def rec(file_name):
        ...
        listtt=recs.values.tolist()
        return listtt
    listtt=rec(song_query)  
    table=f'<!DOCTYPE html><html><head><link rel="stylesheet" type="text/css" href="static/test.css"><title>Music Recommendation</title></head><body>'
    single_char="'"
    nfn=song_query.replace('___',' & ')
    char1="'t "
    char2="'t."
    char3="'s "
    char4="'s."
    ...
    nfn=nfn.replace('_',' ')
    nfn=nfn[:-4]
    table+=f'<h2>Song Recommendations for "{nfn}" <br><audio controls="controls" src="{url_for("static", filename=song_query)}" type="file/mpeg" /><br></h2><table class="center"><thead><tr><th>Song</th><th>Play Song</th></tr></thead><tbody>'
    for i in range (0,9):
        abc=listtt[i][0]
        audiopath=abc
        abc=abc.replace('___',' & ')
